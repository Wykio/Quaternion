...
int main(void)
{
	...
	GLShader test;
	CreateProgram(&test);
	LoadVertexShader(test, "test.vs.glsl");
	LoadFragmentShader(test, "test.fs.glsl");
	LinkProgram(test);

	GLShader testOffset;
	CreateProgram(&testOffset);
	LoadVertexShader(testOffset, "testOffset.vs.glsl");
	LoadFragmentShader(testOffset, "test.fs.glsl");
